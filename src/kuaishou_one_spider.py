...
from kuaishou_search_comment import KuaishouCommentSpider
from kuaishou_trans import KsTransSpider
from kuaishou_user_post import KuaishouPostSpider
...
class MyThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            messagebox.showerror('错误', f'线程执行异常: {str(e)}')


class KuaishouSpiderTool:
    def __init__(self, root):
        self.root = root
        self.root.title('爬快手聚合软件v1.1 | 马哥python说 | 公众号: 老男孩的平凡之路')
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="意见收集", command=self.open_sugg)
...
def create_login_root():
    root_login = tk.Tk()
    root_login.title('爬快手聚合软件v1.1 | 马哥python说')
...
作为软件使用者，您默认接受以上协议条款。感谢理解与支持。如有疑问，请联系作者。"""))
    file_menu.add_command(label="意见收集", command=lambda: webbrowser.open(
        "https://docs.qq.com/sheet/DVGxzT0VVSkVzSW1u?tab=cc8q6g", new=0))
    menu_bar.add_cascade(label="File", menu=file_menu)
    ...
    root_login.protocol("WM_DELETE_WINDOW", lambda: root_login.destroy())
    root_login.mainloop()
